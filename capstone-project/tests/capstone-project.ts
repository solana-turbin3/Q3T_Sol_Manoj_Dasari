...
//     const airdropPromises = [admin, maker, betTaker].map(async (k) => {
//       const signature = await connection.requestAirdrop(k.publicKey, 100 * LAMPORTS_PER_SOL);
...
//     } catch(err) {
//       console.log("Error initializing protocol: ", err);
//       throw err;
//     } 
//   });
...
//       // Derive bet PDA with seed
//       const [betPda, betBump] = PublicKey.findProgramAddressSync(
//         [
//             Buffer.from("bet"),
//             maker.publicKey.toBuffer(),
//             betSeed.toBuffer('le', 8) // Convert to little-endian bytes
//         ],
//         program.programId
//       );

//       // Derive vault pool PDA
//       const [vaultPoolPda, vaultPoolBump] = PublicKey.findProgramAddressSync(
...
//     assert.isNull(betAccountBefore.opponent, "Bet should not have an opponent initially");
//     assert.equal(
//         JSON.stringify(betAccountBefore.status),
//         JSON.stringify({ findingOpponent: {} }),
//         "Initial status should be findingOpponent"
//     );
...
//     // Verify opponent assignment and status
//     assert.equal(betAccount.opponent.toString(), betTaker.publicKey.toString());
//     assert.equal(
//         JSON.stringify(betAccount.status),
//         JSON.stringify({ waitingToStart: {} }),
//         "Status should be waitingToStart"
//     );
...
//     const feesAmount = Math.floor(totalBetAmount * (fees / 10000));
...
// // Add helper function to convert BN to buffer
// {
//   interface BN {
...
// anchor.BN.prototype.toBuffer = function(endian: 'le' | 'be', length: number): Buffer {
